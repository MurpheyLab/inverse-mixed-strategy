...
class DynSystem:
    ...
    def rho_dyn_rev(self, rhot, At, at):
        return At.T @ rhot + at

    def rho_dyn_rev_step(self, rhot, At_at):
        At, at = At_at 
        k1 = self.dt * self.rho_dyn_rev(rhot, At, at)
        k2 = self.dt * self.rho_dyn_rev(rhot + k1/2.0, At, at)
        k3 = self.dt * self.rho_dyn_rev(rhot + k2/2.0, At, at)
        k4 = self.dt * self.rho_dyn_rev(rhot + k3, At, at)
        rhot_new = rhot + (k1 + 2.0*k2 + 2.0*k3 + k4) / 6.0
        return rhot_new, rhot_new
    
    @partial(jit, static_argnums=(0,))
    def rho_dyn_rev_scan(self, rhoT, A_traj, a_traj):
        rho0, rho_traj_rev = scan(
            f=self.rho_dyn_rev_step, init=rhoT,
            xs = (A_traj, a_traj)
        )
        return rho_traj_rev

    @partial(jit, static_argnums=(0,))
    def grad_step(self, rhot, Bt, bt):
        return Bt.T @ rhot + bt

    @partial(jit, static_argnums=(0,))
    def steepest_descent(self, x0, u_traj, extra_traj=None, terminal_extra=None):
        _extra_traj = extra_traj if extra_traj != None else self.extra_traj
        _terminal_extra = terminal_extra if terminal_extra != None else self.terminal_extra
        ...
        rhoT = self.dmdx(xT, _terminal_extra)
        rho_traj_rev = self.rho_dyn_rev_scan(rhoT, A_traj_rev, a_traj_rev)
        rho_traj = rho_traj_rev[::-1]

        dldu_traj = vmap(self.grad_step, in_axes=(0,0,0))(rho_traj, B_traj, b_traj)
        return -1.0 * dldu_traj
    
    @partial(jit, static_argnums=(0,))
    def mig_step(self, rhot, Bt):
        temp_vec = Bt.T @ rhot 
        Sigma = jnp.outer(temp_vec, temp_vec)
        return jnp.linalg.solve(Sigma + self.R.T, temp_vec * self.alpha_d)

    @partial(jit, static_argnums=(0,))
    def sac_descent(self, x0, u_traj, alpha_d=-1.0, extra_traj=None, terminal_extra=None):
        """
        alpha_d <= 0.0
        """
        _extra_traj = extra_traj if extra_traj != None else self.extra_traj
        ...
        mig_traj = vmap(self.mig_step, in_axes=(0,0))(rho_traj, B_traj)
        return mig_traj
